...
CREATE TABLE IF NOT EXISTS `staff_roster` (
  ...
  `status` ENUM('available', 'on-shift', 'off-duty', 'suspended', 'pending_verification', 'active') NOT NULL DEFAULT 'available',
  `credential_status` ENUM('verified', 'expiring', 'expired', 'pending') NOT NULL DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS `staffing_requests` (
  ...
  `recurrence_pattern` ENUM('none', 'daily', 'weekly', 'biweekly') NOT NULL DEFAULT 'none',
  `recurrence_days` VARCHAR(60) NULL,
  `recurrence_end_date` DATE NULL,
  `is_paused` TINYINT(1) NOT NULL DEFAULT 0,
  `client_rating` TINYINT UNSIGNED NULL DEFAULT NULL,
  ...
  `cancellation_fee_applied` TINYINT(1) NOT NULL DEFAULT 0,
  `billing_hourly_rate` DECIMAL(6,2) NULL DEFAULT NULL,
  `hours_billed` DECIMAL(5,2) NULL DEFAULT NULL,
  `invoice_status` ENUM('unbilled', 'pending_approval', 'invoiced', 'paid') NOT NULL DEFAULT 'unbilled',
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `facility_id` VARCHAR(36) NULL,
  `client_role` ENUM('requester', 'billing_admin', 'facility_director') NOT NULL DEFAULT 'requester',
  `phone` VARCHAR(30) NULL,
  `is_active` TINYINT(1) NOT NULL DEFAULT 1,
  `email_verified` TINYINT(1) NOT NULL DEFAULT 0,
  `last_login` TIMESTAMP NULL DEFAULT NULL,
